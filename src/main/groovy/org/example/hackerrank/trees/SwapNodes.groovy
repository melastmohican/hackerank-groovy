 ...
 */
class SwapNodes implements HackerRankExecutable {
    static class Node {
        int data
        Node left
        Node right
    ...
    @Override
    def execute() {
        new Scanner(System.in).withCloseable { scan ->
            Node head = read(scan)

            int t = scan.nextInt()
            while(t--){
                swap(head, scan.nextInt(), 1);
                printInOrder(head)
                println()
            }
        }
    ...
    Node read(Scanner scan) {
        ...
        (1..n).each {
            ...
            if (left != -1) {
                node.left = new Node(left, node.depth + 1)
                queue << node.left
            }
            if (right != -1) {
                node.right = new Node(right, node.depth + 1)
                queue << node.right
            }
        }
        head
    ...
    def printInOrder(Node head) {
        if (head.left)  printInOrder(head.left)
        print head.data + " "
        if (head.right) printInOrder(head.right)
    }

    def swap(Node head, int nodeDepth, int multiplier) {
        if (head) {
